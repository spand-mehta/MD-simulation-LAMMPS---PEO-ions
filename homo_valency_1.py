...
npoly =  400                # number of polymers
dens = 0.5                  # bead density
packing_density = 0.4
bond = 0.97                 # bond length. depends on bond potential, but close to 1 is good enough
ion_conc = 0.3
total_cation=int(ion_conc*npoly*nbeads)
nanion = total_cation
ncation = total_cation #half of anions since charge is double +2.

# dictionary between type and charge (if neutralizedfraction<1, some - charges will be overwritten as 0)
charge  = {1:0, 2:1, 3:-1}  # cation valency is +1

# define monomer
sequence = [1]*int(nbeads)
#/Users/LENOVO/Documents/OSU/Semester\ 4/lammps_bornsolv-master\ 2/src/lammps/initial\ structures
# files
INPUT_LAMMPS = open('input_homoions_0_3_angle_delta_Li.lammps', 'w')

# constants
ntypes = 3 # PEO + anions + cations
ntot = npoly*nbeads
anion_diameter= 1.62
cation_diameter=0.43
#vol = (ntot + nanion + ncation)/dens
vol=(3.14/6)*(ntot+nanion*(anion_diameter**3)+ncation*(cation_diameter**3))/packing_density
...
for i in range(1,ntot):
    #if not at the end of the polymer
    if (molnum[i+1] == molnum[i] and molnum[i+2]==molnum[i+1]):
        iangle = iangle+1 #the angle number
        j=i+1
        k=i+2
